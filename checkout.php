    ...
    <section class="checkout-section spad">
        <div class="container">
            ...
            <form action="#" class="checkout-form">
				...
				<div class="row">
					<div class="col-lg-5">
						<?php echo '<h4>Adresse de facturation</h4>
						<div class="row">
							<select class="form-select" id="selectBillingAddress" onchange="getBillingAddress(this.value); return false;">
								<option value="-1" selected="selected">Séléctionnez une adresse de facturation</option>';
								$getBillingAddresss = $db->query('SELECT * FROM `customers_address` WHERE `customerID`="'.$_SESSION['web_joema']['session_user_id'].'"');
								if($getBillingAddresss->num_rows >= 1 ){
									while($getBillingAddress = $getBillingAddresss->fetch_object()){
										echo '<option value="'.$getBillingAddress->ID.'">'.$getBillingAddress->addressName.'</option>';
									}
								}
								echo '<option value="0">Ajouter une adresse de facturation</option>
								</select>
							</div>';
							echo '<div style="display:none;" id="setBillingAddress"></div>
							<div class="row mt-3" style="display:none;" id="addNewBillingAddress">
								<div class="col-lg-12">
									<label for="fir">Nom de l\'adresse<span>*</span></label>
									<input type="text" id="addBilAddressName" class="form-control">
								</div>
								<div class="col-lg-6">
								...
								<div class="col-lg-12">
									<a href="#" class="btn btn-danger" onclick="toggleNewBillingAddressForm();return false;">Annuler</a>
									<a href="#" class="btn btn-primary pull-right" onclick="addBillingAddress(); return false;">Ajouter</a>
								</div>
							</div>
							<h4 class="mt-5">Adresse de livraison</h4>
							<div class="row">
								<select class="form-select" id="selectDeliveryAddress" onchange="getDeliveryAddress(this.value); return false;">
									<option value="-1" selected="selected">Séléctionnez une adresse de livraison</option>';
									...
									if($getCustomersAddress->num_rows >= 1 ){
										while($getCustomerAddress = $getCustomersAddress->fetch_object()){
											echo '<option value="'.$getCustomerAddress->ID.'">'.$getCustomerAddress->addressName.'</option>';
										}
									}
									echo '<option value="0">Ajouter une adresse de livraison</option>
								</select>
							</div>';
							echo '<div style="display:none;" id="setDeliveryAddress"><input type="hidden" id="userPostal"/></div>
							<div class="row mt-3" style="display:none;" id="addNewAddress">
								<div class="col-lg-12">
									<label for="fir">Nom de l\'adresse<span>*</span></label>
									<input type="text" id="addAddressName" class="form-control">
								</div>
								<div class="col-lg-6">
									<label for="fir">Nom<span>*</span></label>
								...
								<div class="col-lg-12">
									<a href="#" class="btn btn-danger" onclick="toggleNewAddressForm();return false;">Annuler</a>
									<a href="#" class="btn btn-primary pull-right" onclick="addDeliveryAddress(); return false;">Ajouter</a>
								</div>
							</div>'; ?>
					</div>
					<div class="col-lg-7">
						<div class="place-order">
							...
							<div class="order-total">
								...
								<input type="text" class="form-control" id="mondialRelayID"/>
								<div>Nom du relais  = <span id="mr_Name"></span></div>
								<div>Adresse du relais  = <span id="mr_Adress"></span>
								<span id="mr_CP"></span> <span id="mr_City"></span></div>
			...
			<?php } else { ?>
				<div id="checkout-choice-form">
					<div class="row">
						<div class="col-lg-12">
							<a href="account.php" class="btn-checkout-choice">Me connecter</a>
							<a href="account.php?register=true" class="btn-checkout-choice">M'inscrire</a>
							<a href="checkout-register.php" class="btn-checkout-choice">Commander sans inscription</a>
						</div>
					</div>
				</div>
			<?php } ?>
